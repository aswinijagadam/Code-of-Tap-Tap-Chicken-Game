...
public class Level_7_Wedge : MonoBehaviour
{
    ...
    IEnumerator Win()
    {
        ...
        Exit_from_Menu.SetActive(false);
        PlayerPrefs.SetString("Level-9_Open/Closed", "open");
        speed = 0;
    ...
    void Update()
    {
        ...
        if (Time.timeSinceLevelLoad <= Scene_time && Time.timeSinceLevelLoad > timer_time)
        {
            ...
            if (Sun.transform.rotation.x > 0.2f)
            {
                Sun.gameObject.transform.Rotate(-0.01f, 0, 0);
            }
        }
