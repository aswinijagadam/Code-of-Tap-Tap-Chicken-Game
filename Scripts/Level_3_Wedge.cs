...
public class Level_3_Wedge : MonoBehaviour
{
    ...
    IEnumerator Win()
    {
        ...
        Exit_from_Menu.SetActive(false);
        PlayerPrefs.SetString("Level-5_Open/Closed", "open");
        speed = 0;
    ...
    void Update()
    {
        ...
        if (Time.timeSinceLevelLoad <= Scene_time && Time.timeSinceLevelLoad > timer_time)
        {
            ...
            if(Sky.gameObject.transform.position.y > -24)
            {
                Sky.gameObject.transform.position -= new Vector3(0, 0.009f, 0);
            }
            if (Scene_time - Time.timeSinceLevelLoad > 25)
            {
                _light.intensity -= 0.0005f;
            }
            if (Scene_time - Time.timeSinceLevelLoad < 8)
            {
                _light.intensity -= 0.0005f;
            }
        }
